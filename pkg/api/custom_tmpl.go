...
func (c *Controller) listCustomTemplates(request *restful.Request, response *restful.Response) {
	...
	if c.customTmplStore == nil {
		_ = response.WriteHeaderAndJson(http.StatusOK, []store.CustomTemplate{}, restful.MIME_JSON)
		return
	}
...
func (c *Controller) getCustomTemplate(request *restful.Request, response *restful.Response) {
	...
	if c.customTmplStore == nil {
		_ = response.WriteHeaderAndJson(http.StatusNotFound, map[string]string{"error": "custom template store disabled"}, restful.MIME_JSON)
		return
	}
	...
	if ct == nil {
		_ = response.WriteHeaderAndJson(http.StatusNotFound, map[string]string{"error": "custom template not configured for channel"}, restful.MIME_JSON)
		return
	}
	_ = response.WriteHeaderAndJson(http.StatusOK, ct, restful.MIME_JSON)
}
...
func (c *Controller) saveCustomTemplate(request *restful.Request, response *restful.Response) {
	...
	channel := request.PathParameter("channel")
	if c.customTmplStore == nil {
		_ = response.WriteHeaderAndJson(http.StatusBadRequest, map[string]string{"error": "custom template store disabled"}, restful.MIME_JSON)
		return
	}
	...
	if body.Content == "" {
		_ = response.WriteHeaderAndJson(http.StatusBadRequest, map[string]string{"error": "content must not be empty"}, restful.MIME_JSON)
		return
	}
